...
export function buildPrompt({ answers, stories, extraContext, orgName, candid }) {
  const scoreMap = {};
  ...
  const sectorChoice = labelFor("sector", answers);
  const horizonChoice = labelFor("horizon", answers);
  const posture = postureDirection(answers);
  const workspace = labelFor("workspace", answers);
  const chat = labelFor("chat", answers);
...
4. THE FIRST MOVE: one concrete first win to chase. Not three. One. Aim it where they said a win is worth the most ("${valChoice}"), make it native to their stack, and tie it to the real friction or win they described.
