...
- Avoid unverifiable comparative claims ("ahead of most", "better than most companies"). You don't know the benchmark. If you must compare, hedge it ("further along than teams this size often are") or just describe where they are without the comparison.
...
Be careful with claims you can't back. Avoid unverifiable comparatives like "ahead of most" or "better than most companies" unless you soften them honestly (e.g. "further along than teams your size tend to be"). You don't know the real benchmark, so don't assert one. Same with the agentic horizon: only say agents are "running real tasks" if their answers genuinely support it, otherwise describe it more carefully. Honesty is the whole point of this read.
...
${coreTools ? `Their named systems: ${coreTools}. IMPORTANT: reference at least one of these by name somewhere in the read where it's relevant (usually the first move or the honest thing). Use your knowledge of them as a readiness signal. Some (e.g. HubSpot, Salesforce, Snowflake, Notion, modern cloud tools) are AI-friendly and a good base to build on. Others (legacy, on-prem, closed systems) will fight them. Work it in naturally, for example "X is a strong base to build on" or "Y will be the thing that slows you down". Keep it a soft signal that shapes the advice, do NOT use it to re-score their maturity. If a named tool is genuinely unfamiliar to you, don't guess, just skip that one.\n` : ""}
